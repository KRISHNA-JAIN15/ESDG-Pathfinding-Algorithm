...
def run_experiment(dataset_path: str, source_node: str, num_rows: int):
    """Runs experiment: load data -> build ESDG -> run serial FPD."""
    timings = {}
    ...
    # --- Stage 2: Build ESDG + Run Serial Algorithm ---
    start_time = time.perf_counter()
    esd_graph = transform_temporal_to_esd(temporal_edges_list)
    fpd_solver = SerialESDG_FPD(esd_graph)
    fpd_solver.find_fastest_paths(source_node)
    timings['serial_fpd'] = time.perf_counter() - start_time
...
def analyze_scalability():
    """
    Runs experiments and plots results (only data loading + serial algorithm).
    """
    print("="*60)
    print("  RUNNING SCALABILITY ANALYSIS (NO CACHE)")
    print("="*60)

    DATASET_FILE = "Datasets/network_temporal_day.csv"
    SOURCE_VERTEX = "2421"
    dataset_sizes = [1000, 5000, 10000, 15000, 20000, 25000, 30000, 35000, 40000, 45000, 50000, 55000, 60000, 65000, 70000, 75000, 80000, 85000, 90000, 95000, 100000]

    results = []

    for size in dataset_sizes:
        print(f"Running experiment for dataset size: {size} rows")
        timings = run_experiment(DATASET_FILE, SOURCE_VERTEX, size)
        results.append({'size': size, **timings})
    ...
    ax.plot(df['size'], df['data_loading'], marker='^', linestyle='--', label='Data Loading')
    ax.plot(df['size'], df['serial_fpd'], marker='o', linestyle='-', color='blue',
            label='Serial FPD (Build + Solve)')

    ax.set_title('Algorithm Scalability: Data Loading & Serial FPD', fontsize=16)
    ax.set_xlabel('Number of Temporal Edges (Dataset Size)', fontsize=12)
    ax.set_ylabel('Execution Time (seconds)', fontsize=12)
    ax.legend(fontsize=12)
    ax.grid(True)
    ax.set_yscale('log')  # highlight difference on log scale
    plt.savefig("analysis/scalability_no_cache.png")
    plt.show()
